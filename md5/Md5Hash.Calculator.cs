    ...
    partial struct Md5Hash {
        ...
        sealed class Calculator {
            ...
            unsafe struct MD5Context {
                ...
                void FF(ref uint a, uint b, uint c, uint d, uint x, int s, uint Ac) {
                    a = RotateLeft((b & c | ~b & d) + a + x + Ac, s) + b;
                }
                void GG(ref uint a, uint b, uint c, uint d, uint x, int s, uint Ac) {
                    a = RotateLeft((b & d | c & ~d) + a + x + Ac, s) + b;
                }
                void HH(ref uint a, uint b, uint c, uint d, uint x, int s, uint Ac) {
                    a = RotateLeft((b ^ c ^ d) + a + x + Ac, s) + b;
                }
                void II(ref uint a, uint b, uint c, uint d, uint x, int s, uint Ac) {
